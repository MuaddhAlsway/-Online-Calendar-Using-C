...
const char *wdays[] = {"Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"}; // ✅ Semicolon was missing
...
void init_months() {
    ...
    // ✅ Fixed leap year logic and typo
...
void print_row(int row) {
    ...
    for (c = from; c < to; c++) {
        for (i = 0; i < 7; i++)
            printf("%s%s", wdays[i], i == 6 ? "" : " "); // ✅ Removed extra semicolon
        if (c < to - 1) space(gap);
        else putchar('\n');
    }
    ...
    while (1) {
        ...
        space(lead);
        for (c = from; c < to; c++) {
            for (i = 0; i < months[c].start_wday; i++) space(3); // ✅ Fixed `month` typo
...
void print_year() {
    int row;
    char buf[32];
    sprintf(buf, "%d", year); // ✅ You forgot to define `buf`
    space((width - strlen(buf)) / 2);
    printf("%s\n\n", buf);

    for (row = 0; row * cols < 12; row++) // ✅ Removed semicolon that made loop body empty
        print_row(row);
}
...
int main(int c, char **v) {
    ...
    scanf("%d", &year); // ✅ Corrected format string
